...
export class ChunkSyncManager<T = unknown> {
  ...
  private readonly retryQueue: Array<{ chunk: SyncChunk<T>; attempts: number; nextRetry: number }> = [];
  ...
  receiveChunk(incoming: SyncChunk<T>): { accepted: boolean; resolution?: ChunkConflictResolution<T> } {
    // Advance local Lamport clock
    this.currentLamport = Math.max(this.currentLamport, incoming.lamportTimestamp) + 1;
  ...
  getPendingRetries(): SyncChunk<T>[] {
    ...
    return this.retryQueue
      ...
      .map((item) => {
        item.attempts += 1;
        item.nextRetry = now + this.retryDelayMs * Math.pow(2, item.attempts);
        return item.chunk;
      });
  }
