...
import type { EdgeMeshConfig, EdgeMeshEventMap, NodoId } from "./types/index.js";
import { TIPO_MENSAJE } from "./types/index.js";
import { createEdgeMeshNode, type EdgeMeshNode } from "./core/node.js";
import { createEnvelope, MessageDeduplicator, validateEnvelope } from "./protocol/index.js";
import { StorageManager, InMemoryStorage } from "./storage/index.js";
import { PeerJSTransport, type PeerJSTransportOptions } from "./transport/peerjs.js";
import { GovernanceManager, createGovernanceManager } from "./governance/index.js";
import { createPostQuantumIdentity, generateKeypair, type PostQuantumIdentity, type PostQuantumKeypair } from "./identity/index.js";
import { PresenceManager } from "./presence/index.js";
import { NamespaceAuthorizer, createNamespaceAuthorizer } from "./authz/index.js";
...
export class YjsAdapter {
  readonly doc: Y.Doc;
  private readonly listeners: Map<string, Set<(update: Uint8Array, origin: unknown) => void>>;
...
export class EdgeMesh {
  ...
  private async procesarSync(env: ReturnType<typeof createEnvelope>): Promise<void> {
    const payload = env.payload as { docId: string; datos: Uint8Array; clock: number };
  ...
  private async procesarSnapshot(env: ReturnType<typeof createEnvelope>): Promise<void> {
  ...
  private async procesarGovernance(env: ReturnType<typeof createEnvelope>): Promise<void> {
  ...
  private async procesarAuthz(env: ReturnType<typeof createEnvelope>): Promise<void> {
    const payload = env.payload as {
      accion: string;
      espacio: string;
      sujeto: NodoId;
  ...
  private async procesarNamespace(env: ReturnType<typeof createEnvelope>): Promise<void> {
    const payload = env.payload as {
      accion: string;
      espacio: string;
      nodoId: NodoId;
