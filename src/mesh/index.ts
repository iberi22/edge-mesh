...
import type { NodoId, Envolvente } from "../types/index.js";
...
export interface MeshEventMap {
  peerConectado: CustomEvent<{ readonly peerId: NodoId; readonly namespace?: string }>;
  peerDesconectado: CustomEvent<{ readonly peerId: NodoId }>;
  peerDescubierto: CustomEvent<{ readonly peerId: NodoId; readonly via: NodoId }>;
  gossipRecibido: CustomEvent<{ readonly mensaje: GossipMessage }>;
  meshSaludActualizada: CustomEvent<{ readonly peersActivos: number; readonly peersTotales: number }>;
  namespaceSincronizado: CustomEvent<{ readonly namespace: string; readonly peers: readonly NodoId[] }>;
...
export class MeshManager extends EventTarget {
  ...
  private intervalos: { heartbeat?: ReturnType<typeof setInterval>; limpieza?: ReturnType<typeof setInterval> } = {};

  constructor(config: Partial<MeshConfig> & { nodoId: NodoId }, edgeMesh: EdgeMesh) {
    super();
  ...
  private seleccionarPeersParaFanOut(
    ...
    const disponibles = candidatos.filter(
      (p) => p !== this.config.nodoId && !excluir.includes(p) && this.peers.get(p)?.estado === "activo",
    );
  ...
  private seleccionAleatoria(peers: readonly NodoId[], count: number): NodoId[] {
  ...
  private seleccionPorLatencia(peers: readonly NodoId[], count: number): NodoId[] {
  ...
  private async reenviarGossip(peerId: NodoId, mensaje: GossipMessage): Promise<void> {
    try {
      const env = createEnvelope(
        TIPO_MENSAJE.GOVERNANCE as never,
        this.config.nodoId,
  ...
  procesarHeartbeatPeer(
    ...
    const existente = this.peers.get(peerId);
    if (existente !== undefined) {
      this.peers.set(peerId, { ...existente, ultimoHeartbeat: Date.now(), estado: "activo", intentosReconexion: 0 });
    } else {
