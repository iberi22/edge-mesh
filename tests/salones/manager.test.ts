...
describe("SalonVirtual", () => {
	...
	it("debería re-enviar mensajes del chat", async () => {
		...
		const yjsMensajes = edgeMesh.yjsAdapter.getArray(
			`chat:salon:${salon.id}:mensajes`,
		);
		yjsMensajes.push([
			{
				id: "msg-remote",
...
describe("SalonesManager", () => {
	...
	it("solo el creador debería poder cerrar el salón", async () => {
		...
		const otherPeerEdgeMesh = new EdgeMesh({
			nodoId: "peer2" as NodoId,
			storageBackend: "mem",
		});
		...
		await expect(otherManager.cerrarSalon(salon.id)).rejects.toThrow(
			/Solo el creador/,
		);
