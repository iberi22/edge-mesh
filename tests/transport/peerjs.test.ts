...
describe("PeerJSTransport", () => {
  ...
  it("should forward PeerJS events", () => {
    ...
    const openHandler = peerInstance.on.mock.calls.find((call: any) => call[0] === "open")[1];
    openHandler("peer-1");
    ...
    const errorHandler = peerInstance.on.mock.calls.find((call: any) => call[0] === "error")[1];
  ...
  it("should handle incoming connections", () => {
    ...
    const connectionHandler = peerInstance.on.mock.calls.find((call: any) => call[0] === "connection")[1];
    connectionHandler(mockConn);

    expect(conectadoSpy).toHaveBeenCalledWith(expect.objectContaining({
