...
import { InMemoryStorage, StorageManager, StorageError } from "../../src/storage/index.js";
...
describe("Storage Module", () => {
  ...
  describe("StorageManager (IndexedDB)", () => {
    ...
    beforeEach(() => {
      ...
      storage = new StorageManager({ dbName: "test-db", storeName: "test-store" });
    });
    ...
    it("should set and get values", async () => {
      ...
      mockDb.get.mockResolvedValueOnce({ key: "k1", valor: "v1", version: 1, timestamp: 123 }); // call to get()

      await storage.set("k1", "v1");
      expect(mockDb.put).toHaveBeenCalledWith("test-store", expect.objectContaining({
        key: "k1",
        valor: "v1",
        version: 1
      }));
