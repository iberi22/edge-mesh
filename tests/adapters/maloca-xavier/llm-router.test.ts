...
describe("LLMRouterAdapter", () => {
  ...
  it("should route prompt to the best available provider", async () => {
    ...
    vi.spyOn(edgeMesh.presence, "obtenerSalud").mockImplementation((id) => {
      if (id === provider1) {
        return { nodoId: provider1, estado: "saludable", ultimoHeartbeat: Date.now(), latenciaMs: 50, fallosConsecutivos: 0 };
      }
      if (id === provider2) {
        return { nodoId: provider2, estado: "saludable", ultimoHeartbeat: Date.now(), latenciaMs: 200, fallosConsecutivos: 0 };
      }
      return null;
    });
    ...
    expect(enviarSpy).toHaveBeenCalledWith(provider1, expect.objectContaining({
      tipo: "xavier:prompt",
      prompt: "Hola Xavier",
      model: "gpt-4",
    }));
  ...
  it("should return null if no provider supports the model", async () => {
    const result = await routerAdapter.routePrompt("Hola", "non-existent-model");
  ...
  it("should share context with multiple agents", async () => {
    ...
    expect(enviarSpy).toHaveBeenCalledWith("agent-1", expect.objectContaining({
      tipo: "xavier:context",
