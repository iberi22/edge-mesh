...
import { NamespaceManager, NAMESPACE_POR_DEFECTO } from '../../src/namespaces/index.js';
...
describe('NamespaceManager', () => {
  ...
  it('debería evitar eliminar el namespace por defecto', () => {
    ...
    expect(manager.obtenerEspacioPorNombre(NAMESPACE_POR_DEFECTO)).not.toBeNull();
  });
  ...
  it('debería permitir unir y abandonar nodos', () => {
    ...
    expect(manager.obtenerEspaciosDeNodo(nodoId)).toContainEqual(expect.objectContaining({ nombre: 'test' }));
  ...
  it('debería disparar eventos al crear/eliminar espacios', () => {
    ...
    expect(creadoListener.mock.calls[0][0].detail.espacio.nombre).toBe('event-test');
