...
describe('PayloadOptimizer', () => {
  it('debería comprimir y descomprimir un payload complejo manteniendo la fidelidad', async () => {
    const originalPayload = {
      id: 'doc_123',
      content: 'Este es un texto de prueba que se va a comprimir con Zstd y serializar con CBOR.',
      timestamp: Date.now(),
      tags: ['test', 'cbor', 'zstd'],
      metadata: {
        active: true,
    ...
    // Compress
    const compressedData = await PayloadOptimizer.compressPayload(originalPayload);
    ...
    // Decompress
    const recoveredPayload = await PayloadOptimizer.decompressPayload<typeof originalPayload>(compressedData);
